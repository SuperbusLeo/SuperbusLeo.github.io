...
import { VariableParseAndCheck , DependentDeclarataion , SkipDivideByZero } from './Helper.js';
import { stateAbbList, stateList, checkList } from './Other_Data.js';
import { stateToAbbreviation , WCClassCheck , SUIConverter } from './Converter_Functions.js'
...
export function runMath() {
  ...
  var FINANCE;
  var creditStatement;
  if (paymentTerms.value === "Net 0") {
  	FINANCE = 0;
  	creditStatement = "No Credit App Needed";
  } else if (paymentTerms.value === "Net 7") {
  	FINANCE = 0.0025;
  	creditStatement = "Credit App Needed";
  } else if (paymentTerms.value === "Net 14" || paymentTerms.value === "Net 15" || paymentTerms.value === "Net 20" || paymentTerms.value === "Net 21") {
  	FINANCE = 0.005;
  	creditStatement = "Credit App Needed";
  } else if (paymentTerms.value === "Net 25") {
  	FINANCE = 0.0075;
  	creditStatement = "Credit App Needed";
  } else if (paymentTerms.value === "Net 30") {
  	FINANCE = 0.01;
  	creditStatement = "Credit App Needed";
  } else if (paymentTerms.value === "Net 45") {
  	FINANCE = 0.0175;
  	creditStatement = "Credit App Needed,CFO Approval";
  } else if (paymentTerms.value === "Net 60") {
  	FINANCE = 0.03;
  	creditStatement = "Credit App Needed,CFO & CEO Approval";
  }
  ...
  //document.getElementById('creditLimit').textContent = "Credit Limit Needed: " + result; //This is something that was only there for viewing, it was removed as it was deemed pointless for scaling.
	...
	//HTML modifiers
	document.getElementById('weeklyEstRevenue').textContent = `Weekly Est. Revenue: ${weeklyEstimateRevenue}`;
	document.getElementById('simpleGP').textContent = `Simple GP: ${simpleGPNumber}%`;
	document.getElementById("weightGPHour").textContent = `GP Weighted Per Hour: ${gpWeightedPerHour}`;
	document.getElementById("weightGPWeek").textContent = `GP Weighted Per Week: ${gpWeightedPerWeek}`;
	document.getElementById("payRateRepeat").textContent = `Pay Rate: ${payRateVar}`;
	document.getElementById("billRate").textContent = `Bill Rate: ${billRateVar}`;
	document.getElementById("markupRepeat").textContent = `Mark-up ${markUpVar}`;
	document.getElementById("trueGPPercent").textContent = `True Gross Profit: ${trueGrossProfitPercent}%`;
	document.getElementById("trueGPDollars").textContent = `True Gross Profit: $${trueGrossProfitDollar}`;
	document.getElementById("weeksToProfit").textContent = `Weeks to Profit: ${weeksToProfitVar}`;
	document.getElementById("weeksToWageLimit").textContent = `Weeks to SUTA Wage Limit: ${weeksToSUTAWageLimit}`;
	document.getElementById("hoursToProfit").textContent = `Hours to profit per EE: ${hoursToProfitPerEE}`
}
...
export function saveFile() {
  ...
  for (const input of allIds) {
    ...
    if (!tempVal.value) {
      ...
      document.getElementById('errorMessage').textContent = "Missing information! Look at the highlighted Red Boxes to make sure you filled in the correct information!";
      doIContinue = false;
    } else {
